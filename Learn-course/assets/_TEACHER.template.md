...
3. If the lesson directory has `animation.html`, open it and ask the learner to step through it before the first exercise. Skip this when the file is absent.
4. List the exercises and tell the learner the path to the first uncompleted one. Wait.
5. When the learner says "check" / "I'm done" / "test it":
...
6. If `{{test_command}}` is not installed locally, say so and fall back to a read-only review by comparing the learner's file to `.solutions/NN-slug.solution.<ext>`. Label it clearly as a review, not a test result.
